function simtools_Inport_to_goto(varargin)
% simtools_Inport_to_goto - 将选中的Inport模块替换为Goto/From模式
% 
% 语法:
%   simtools_Inport_to_goto()           % 自动转换当前选中的Inport模块
%   simtools_Inport_to_goto(block_paths) % 转换指定的模块
%
% 功能说明:
%   1. 自动获取当前Simulink模型中选中的Inport模块
%   2. 为每个选中的Inport创建对应的Goto和From模块
%   3. Inport模块被替换为From模块，From模块连接到Goto模块
%   4. 保持原有信号流不变
%   5. 验证连接是否完整
...
if nargin == 0
    ...
    try
        ...
        % 获取所有Inport模块并检查哪些被选中
        all_inports = find_system(current_system, 'BlockType', 'Inport');
        
        % 检查哪些被选中
        block_paths = {};
        for i = 1:length(all_inports)
            block = all_inports{i};
            try
        ...
        if isempty(block_paths)
            uiwait(msgbox('未找到选中的Inport模块！请在模型中选中至少一个Inport模块后重试。', '提示', 'warn'));
            return;
        end
        
        % 构建显示信息
        msg = sprintf('找到 %d 个选中的Inport模块：\n\n', length(block_paths));
...
if CONFIG.SHOW_PROGRESS
    hWaitBar = waitbar(0, '正在转换Inport模块...', 'Name', '转换进度');
end
...
for i = 1:length(block_paths)
    ...
    try
        ...
        if ~strcmp(block_type, 'Inport')
            warning('模块 %s 不是Inport类型（实际为 %s），跳过！', current_block, block_type);
            fail_count = fail_count + 1;
            continue;
        end
        ...
        % 获取端口号 - 修复：Port可能是字符串或数值
        try
            port_number = get_param(current_block, 'Port');
            if ischar(port_number)
                port_number = str2double(port_number);
            end
            if isnan(port_number) || isempty(port_number)
                port_number = i;  % 使用索引作为默认值
            end
        catch
            port_number = i;  % 如果获取失败，使用索引
        end
        
        fprintf('\n--- 转换模块: %s ---\n', block_name);
        fprintf('端口号: %d\n', port_number);
        
        %% 步骤1：保存原Inport模块的所有连接信息
        port_handles = get_param(current_block, 'PortHandles');
        
        % 处理输入端口（如果有）
        if isfield(port_handles, 'Inport') && ~isempty(port_handles.Inport)
            inport = port_handles.Inport(1);
            input_line = get_param(inport, 'Line');
            ...
            if input_line ~= -1
                try
                    src_handle = get_param(input_line, 'SrcPortHandle');
                    if ishandle(src_handle) && src_handle ~= -1
                        src_block = get_param(src_handle, 'Parent');
                        src_block_path = getfullname(src_block);
                        src_port_num = get_param(src_handle, 'PortNumber');
                        fprintf('输入源: %s (端口 %d)\n', src_block_path, src_port_num);
                    end
                catch
                    % 忽略
                end
        ...
        % 处理输出端口
        if isfield(port_handles, 'Outport') && ~isempty(port_handles.Outport)
            outport = port_handles.Outport(1);
            output_line = get_param(outport, 'Line');
        else
            outport = [];
            output_line = -1;
        end
        ...
        if output_line ~= -1
            % 获取所有目标端口
            dst_handles = get_param(output_line, 'DstPortHandle');
        ...
        %% 步骤2：删除原Inport模块的输出线（关键步骤！）
        if output_line ~= -1
            try
                delete_line(output_line);
                fprintf('  ✓ 已删除原始输出连接线\n');
            catch ME
                fprintf('  ⚠ 删除原始输出连接线失败：%s\n', ME.message);
            end
        end
        
        % 步骤2.5：如果有输入线，也断开（但保留输入源信息用于后续连接）
        if input_line ~= -1
            try
                delete_line(input_line);
                fprintf('  ✓ 已删除原始输入连接线\n');
            catch ME
                fprintf('  ⚠ 删除原始输入连接线失败：%s\n', ME.message);
            end
        end
        ...
        %% 计算模块尺寸（根据名字长度自动调整宽度）
        max_name_length = max([length(goto_name), length(from_name)]);
        ...
        %% 步骤5：连接外部输入到Goto
        % 如果有外部输入源，将其连接到Goto模块
        if ~isempty(src_block_path)
            try
                % 验证输入源模块是否存在
                try
                    get_param(src_block_path, 'Handle');
                    src_exists = true;
                catch
                    src_exists = false;
                end
                ...
                if src_exists
                    % 重新获取输入源的输出端口
                    src_block_ports = get_param(src_block_path, 'PortHandles');
                    if isfield(src_block_ports, 'Outport') && ~isempty(src_block_ports.Outport)
                        % 使用保存的端口号或默认端口1
                        if ~isempty(src_port_num) && src_port_num <= length(src_block_ports.Outport)
                            src_out = src_block_ports.Outport(src_port_num);
                        else
                            src_out = src_block_ports.Outport(1);
                        end
                        goto_ports = get_param(goto_block, 'PortHandles');
                        if isfield(goto_ports, 'Inport') && ~isempty(goto_ports.Inport)
                            goto_inport = goto_ports.Inport(1);
                            add_line(parent_system, src_out, goto_inport);
                            fprintf('  ✓ 连接: 外部输入 -> Goto\n');
                        end
                    end
                else
                    fprintf('  ⚠ 输入源模块已不存在，请手动连接\n');
                end
            catch ME
                fprintf('  ⚠ 连接外部输入到Goto失败：%s\n', ME.message);
                fprintf('  ⚠ 请手动连接外部输入到 %s\n', goto_block);
            end
        else
            fprintf('  (无外部输入，Goto将作为顶层输入使用)\n');
            % 对于顶层输入，不需要连接外部信号
        end
        
        %% 步骤6：连接From -> 原来的后续模块
        from_ports = get_param(from_block, 'PortHandles');
        if isfield(from_ports, 'Outport') && ~isempty(from_ports.Outport)
            from_out = from_ports.Outport(1);
        else
            error('From模块没有输出端口！');
        end
        ...
        verification_results.(block_name).port_number = port_number;
        ...
        if CONFIG.AUTO_ARRANGE
            try
                Simulink.BlockDiagram.arrangeSystem(parent_system, ...
                    'Selected', {goto_block, from_block});
            catch
                % 如果自动整理失败，忽略
            end
        end
...
if CONFIG.VERIFY_CONNECTIONS
    ...
    for i = 1:length(block_names)
        ...
        if result.status
            ...
            fprintf('  ✓ %s (端口 %d): 连接完整 (%d/%d)\n', ...
                block_name, result.port_number, result.connected, result.expected);
        else
            verification_failed = verification_failed + 1;
            fprintf('  ✗ %s (端口 %d): 连接不完整 (%d/%d)\n', ...
                block_name, result.port_number, result.connected, result.expected);
        end
    end
